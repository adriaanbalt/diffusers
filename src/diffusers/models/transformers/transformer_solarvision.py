...
"""
SolarVision Transformer2DModel for HuggingFace Diffusers.
"""
...
@dataclass
class SolarVisionTransformer2DModelOutput(BaseOutput):
    sample: torch.Tensor
...
class SolarVisionEmbedND3(nn.Module):
...
class SolarVisionPatchEmbedDynamic(nn.Module):
...
class SolarVisionSingleStreamAttnProcessor:
    ...
    def __init__(self):
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError(
                "SolarVisionSingleStreamAttnProcessor requires PyTorch 2.0. To use it, please upgrade PyTorch to version 2.0 or higher."
            )
...
class SolarVisionAttention(torch.nn.Module, AttentionModuleMixin):
    _default_processor_cls = SolarVisionSingleStreamAttnProcessor
...
class SolarVisionFeedForward(nn.Module):
...
class SolarVisionSharedAdaLNBlock(nn.Module):
    def __init__(
        ...
        self.self_attention = SolarVisionAttention(
            ...
            bias=False,
            out_bias=False,
            processor=SolarVisionSingleStreamAttnProcessor(),
        )
        self.adaLN_mlp_ln = RMSNorm(hidden_size, eps=eps)
        self.mlp = SolarVisionFeedForward(hidden_size, ffn_hidden_size)
...
class SolarVisionAdaLNContinuous(nn.Module):
...
class SolarVisionTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    _supports_gradient_checkpointing = True
    _repeated_blocks = ["SolarVisionSharedAdaLNBlock"]
    ...
    def __init__(
        ...
        self.x_embedder = SolarVisionPatchEmbedDynamic(in_channels, hidden_size, patch_size)
        ...
        self.pos_embed = SolarVisionEmbedND3(dim=self.head_dim, theta=rope_theta, axes_dim=rope_axes_dim)
        self.adaLN_modulation = nn.Sequential(nn.SiLU(), nn.Linear(hidden_size, 6 * hidden_size))
        nn.init.zeros_(self.adaLN_modulation[-1].weight)
        nn.init.zeros_(self.adaLN_modulation[-1].bias)
        self.layers = nn.ModuleList(
            [
                SolarVisionSharedAdaLNBlock(
                    hidden_size, num_attention_heads, ffn_hidden_size, eps, qk_layernorm=qk_layernorm
                )
                for _ in range(num_layers)
            ]
        )
        self.final_norm = SolarVisionAdaLNContinuous(hidden_size, eps)
    ...
    def forward(
        ...
        """
        The [`SolarVisionTransformer2DModel`] forward method.
        ...
        return SolarVisionTransformer2DModelOutput(sample=output) if return_dict else (output,)
